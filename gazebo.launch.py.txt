...
def generate_launch_description():
    ...
    slam_params_file = os.path.join(pkg_share, 'config', 'slam_toolbox.yaml')  # ✅ added
    ...
    # --- SLAM Toolbox Node (added) ---
    slam_toolbox = Node(
        package='slam_toolbox',
        executable='async_slam_toolbox_node',
        name='slam_toolbox',
        output='screen',
        parameters=[slam_params_file, sim_time_param]
    )

    # --- Delayed spawning sequence ---
    delayed_steering_spawner = TimerAction(period=5.0, actions=[steering_spawner])
    delayed_wheel_spawner = TimerAction(period=7.0, actions=[wheel_spawner])
    ...
    return LaunchDescription([
        ...
        spawn_entity,
        jsb_spawner,
        delayed_steering_spawner,
        delayed_wheel_spawner,
        slam_toolbox  # ✅ added at the end
    ])
